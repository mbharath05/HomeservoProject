...
import com.homeservo.Exception.EmailNotFoundForVendor;
import com.homeservo.Exception.NoSuchElementFoundInCustomer;
import com.homeservo.Exception.NoSuchElementFoundInVendor;
import com.homeservo.Exception.PasswordIncorrectForVendor;
...
public class VendorService {
	...
	@Autowired
	com.homeservo.duplicate.Vendor vendor1;
	...
	public ResponseEntity<ResponseStructure<com.homeservo.duplicate.Vendor>> getById(int id) {
		Vendor vendor = vendorDao.findByIdVendor(id);
		if (vendor != null) {
			ResponseStructure<com.homeservo.duplicate.Vendor> responseStructure = new ResponseStructure<>();
			...
			responseStructure.setStatus(HttpStatus.FOUND.value());
			return new ResponseEntity<ResponseStructure<com.homeservo.duplicate.Vendor>>(responseStructure, HttpStatus.FOUND);
		} else {
			throw new NoSuchElementFoundInVendor("please check the id properly in vendor");
		}
	}
